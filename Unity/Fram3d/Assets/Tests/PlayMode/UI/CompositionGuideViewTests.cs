...
namespace Fram3d.Tests.UI
{
    ...
    public sealed class CompositionGuideViewTests
    {
        ...
        public IEnumerator Start__AllElementsIgnoreInput__When__Initialized()
        {
            ...
            for (var i = 0; i < container.childCount; i++)
                Assert.AreEqual(PickingMode.Ignore, container[i].pickingMode,
                    $"Element {i} should ignore mouse events");
        }
        ...
        public IEnumerator ToggleThirds__ShowsThirdsElements__When__Enabled()
        {
            ...
            for (var i = 0; i < 4; i++)
                Assert.AreEqual(DisplayStyle.Flex, container[i].style.display.value,
                    $"Thirds line {i} should be visible");
        }
        ...
        public IEnumerator ToggleAll__ShowsAllGuides__When__NoneVisible()
        {
            ...
            for (var i = 0; i < container.childCount; i++)
                Assert.AreEqual(DisplayStyle.Flex, container[i].style.display.value,
                    $"Element {i} should be visible after ToggleAll");
        }
        ...
        public IEnumerator ToggleAll__HidesAllGuides__When__SomeVisible()
        {
            ...
            for (var i = 0; i < container.childCount; i++)
                Assert.AreEqual(DisplayStyle.None, container[i].style.display.value,
                    $"Element {i} should be hidden after ToggleAll");
        }
