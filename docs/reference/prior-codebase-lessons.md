...
**Software cursor overlay (supersedes native NSView approach).** All native cursor code (MacOSCursorService, EditorCursorService, LinuxCursorService, WindowsCursorPatch, CursorWrapper.dylib, NativeCursorsWebGL.jslib) was deleted in FRA-52. The replacement is `SoftwareCursorOverlay` — a MonoBehaviour that renders cursor PNGs via `GUI.DrawTexture` in `OnGUI` while hiding `Cursor.visible`. This sidesteps the entire macOS cursor rect system. The native overlay NSView approach (documented below as history) worked in standalone but was unreliable in the Editor because Unity's repaint inserts subviews that break cursor rect priority. The software overlay works identically in all contexts with zero platform-specific code.

**[Historical] Transparent overlay NSView for macOS cursor management.** `CursorWrapper.mm` added a `Fram3dCursorOverlayView` — a custom NSView positioned as the topmost subview of the player window's `contentView`. The overlay installed an `NSTrackingArea` with `NSTrackingCursorUpdate`, implemented `cursorUpdate:`. This approach worked in standalone builds but failed in the Editor — Unity's repaint cycle inserts its own subviews above the overlay, breaking cursor rect priority. Even raising the overlay each frame via `addSubview:positioned:relativeTo:` with `NSWindowAbove` couldn't keep up with Editor repaints. This code was deleted and replaced with the software overlay approach above.

---

## Anti-patterns to Avoid

**Overriding macOS cursors from ANY native or managed approach.** Eight approaches were tried and all failed before the problem was sidestepped entirely with a software cursor overlay. The fundamental issue: macOS resets the cursor on every `NSMouseMoved` via its cursor rect system, and Unity's Editor repaint inserts subviews that break cursor rect priority. Approaches that failed: (1) `EditorApplication.update` + `[NSCursor set]` — fires before repaint, cursor reset during repaint. (2) `Cursor.SetCursor()` — overridden by Editor's IMGUI cursor management. (3) `PointerMoveEvent` callback + `[NSCursor set]` — cursor still flickers between tracking area evaluation and callback. (4) `[NSWindow disableCursorRects]` — Unity may re-enable during repaint. (5) `disableCursorRects` + continuous re-apply — still one-frame flashes. (6) Transparent overlay NSView with `cursorUpdate:` + `NSTrackingArea` — worked in standalone but Editor repaints insert subviews above it. (7) Raising overlay NSView each frame via `addSubview:positioned:relativeTo:` — couldn't keep up with Editor repaint frequency. (8) Belt-and-suspenders of all above. **The lesson: don't fight the platform's cursor system. Sidestep it.** The software overlay (`SoftwareCursorOverlay`) hides `Cursor.visible` and renders via `GUI.DrawTexture` — zero platform-specific code, works everywhere, cannot flicker.
