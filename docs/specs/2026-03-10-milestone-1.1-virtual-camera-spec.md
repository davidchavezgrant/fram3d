...
- ### 1.1. Virtual camera (Milestone)
  ...
  - ##### 1.1.1. Camera movement (Feature)
    ...
    **Functional requirements:**
    ...
    - Orbit simultaneously rotates the camera around a target point while keeping that point centered in frame. Orbit respects both horizontal and vertical axes.
  ...
  - ##### 1.1.2. Lens system (Feature)
    ...
    **Functional requirements:**
    ...
    - Focal length is a keyframeable property. It is recorded by auto-keyframing and interpolated during playback.
  ...
  - ##### 1.1.3. Camera body and lens presets (Feature)
    ***Select from real cinema camera bodies and lens sets. The body determines the sensor -- which determines how every focal length looks. The lens set determines the available focal lengths to snap between.***

    **Functional requirements:**
    - Camera body presets, each defining a sensor width and height in millimeters:
      ...
      - Super 16mm
      - 8mm
    - Lens set presets, each defining a list of available prime focal lengths:
      - Zeiss Master Prime: 14, 18, 21, 25, 28, 32, 35, 40, 50, 65, 75, 85, 100, 135, 150mm
      - Cooke S4/i: 14, 18, 21, 25, 27, 32, 35, 40, 50, 65, 75, 100, 135, 150, 180mm
      - Leica Summilux-C: 16, 18, 21, 25, 29, 35, 40, 50, 65, 75, 100, 135mm
      - Sigma Cine FF: 14, 20, 24, 28, 35, 40, 50, 85, 105, 135mm
  ...
  - ##### 1.1.5. Depth of field preview (Feature)
    ...
    **Functional requirements:**
    ...
    - Aperture is adjustable across a cinematically useful range: f/1.4 to f/22. This range covers the creative sweet spot -- from shallow wide-open looks to deep-focus compositions.
    ...
    **Expected behavior:**
    ``` python
      ...
      # aperture affects DOF
      .if DOF preview is on using f/5.6
      ||> .if user opens aperture to f/1.4 >>
          <== DOF becomes much shallower
          <== only objects very near the focus distance remain sharp
          <== background and foreground blur increases noticeably

      .if DOF preview is on using f/1.4
      ||> .if user closes aperture to f/22 >>
          <== DOF becomes very deep
          <== nearly every object within the scene appears sharp
      ...
      .if user adjusts aperture while viewing the scene >>
          <== blur amount updates every frame during adjustment
  ...
  - ##### 1.1.6. Camera shake (Feature)
    ...
    **Functional requirements:**
    ...
    - Shake is cosmetic only -- it is not recorded by auto-keyframing. It is a preview effect, not part of the camera animation data.
    ...
    **Expected behavior:**
    ``` python
      ...
      .if shake is enabled
      ||> .if auto-keyframing records a camera keyframe >>
          <== the recorded position and rotation reflect the true camera state, not the shaken state
    ```

  - ##### 1.1.7. Camera info HUD (Feature)
    ***A persistent overlay showing the data a DP would read off an on-set monitor: focal length, camera height, angle of view, aspect ratio, body, and lens set. Glanceable, not intrusive.***
    ...
    **Functional requirements:**
    ...
    - The HUD renders above all scene content and overlays. It is always legible regardless of the scene behind it.
    - The HUD is always visible. There is no toggle to hide it.
