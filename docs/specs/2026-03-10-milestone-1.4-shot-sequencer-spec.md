...
***A project contains one or more scenes. Each scene has its own global object timeline, its own objects, and its own shot sequence. Scenes are the top-level organizational unit — a new scene for each location, sequence, or alternate version (flashback, dream sequence).***

### Scene Model

- A project starts with one scene (unnamed, or "Scene 1")
- Each scene is independent: its own objects, global object timeline, shots, and camera animations
- Objects, keyframes, and links do NOT carry across scenes — each scene is a self-contained world
- The sequencer shows the current scene's shots. Switching scenes loads that scene's shot sequence

### Scene Creation

``` python
  .if >> user selects File > New Scene
      <== a new empty scene is created
      <== the scene contains no objects, no shots
      <== the user is switched to the new scene
      <== the sequencer clears and shows the "Add Shot" button

  .if >> user selects File > Duplicate Scene
      <== a copy of the current scene is created
      <== the copy includes all objects, their global timeline keyframes, all shots, all camera animations, and all link/group state
      <== the copy is independent — editing it does not affect the original
      <== the user is switched to the new scene
      <== the new scene is named "[Original Name] (Copy)" or "Scene_02" if unnamed
```

### Scene Switching

``` python
  .if >> user switches to a different scene (via tab, dropdown, or menu)
      <== the viewport loads the new scene's objects and state
      <== the sequencer loads the new scene's shots
      <== the timeline loads the new scene's global object timeline
      <== the previous scene's state is preserved (switching back restores it exactly)
      !== objects from the previous scene are visible
      !== camera animations from the previous scene are active

  .if >> user switches scenes during playback
      <== playback stops
      <== the new scene loads at its last-viewed state
```

### Scene Deletion

``` python
  .if >> user deletes a scene that is not the only scene
      <== confirmation dialog: "Delete [scene name]? This cannot be undone."
      <== after confirmation, the scene and all its contents are removed
      <== the user is switched to the nearest remaining scene

  .if >> user attempts to delete the only remaining scene
      <== operation rejected — a project must have at least one scene
```

### Use Cases

- **Flashback:** Duplicate the current scene, reposition characters to the flashback setting. The main scene is unaffected.
- **Dream sequence:** Duplicate or create new. Rearrange the world however you want.
- **Alternate blocking:** Duplicate the scene to try a different staging approach. Compare by switching between scenes.
- **Multiple locations:** Each location in the script is its own scene with its own objects and blocking.
