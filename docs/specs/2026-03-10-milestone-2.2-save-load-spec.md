  ...
  *Blocked by: 2.1 (undo/redo — save integrates with undo stack for dirty tracking)*
  ...
  - ##### 2.2.1. Project file format (Feature)
    ...
    **Functional requirements:**
    ...
    - Scene objects are saved with embedded asset data — the project file contains the asset geometry, not just a file path reference. This means projects are self-contained and portable.
    - When saving, if any embedded asset exceeds a size threshold (threshold TBD — reference: Mixamo X Bot is ~1700KB), the application warns the user about large assets and offers a per-asset choice: bundle the asset into the project file, or reference it externally. This is a save-time prompt, not a blocking error. The exact threshold and UX for this choice are design decisions to be resolved before implementation.
    ...
    **Expected behavior:**
    ``` python
      ...
      # embedded assets are self-contained
