	...
	- ##### 3.1.2. Light properties (Feature)
		...
		**Functional requirements:**
		- Every light has the following base properties:
			...
			- Color — the color of the light, expressed as a hue (warm tungsten, cool daylight, colored gels)
			- The default color is warm white (approximating tungsten)
		...
		- Properties are editable through two methods:
			- Inspector panel: numeric fields for intensity and angles, a color picker for color, a slider for range
		...
		- Common color temperature presets are available:
			- Candlelight (warm orange, ~1800K equivalent)
			- Tungsten (warm white, ~3200K equivalent)
			- Daylight (neutral white, ~5600K equivalent)
			- Overcast (cool white, ~6500K equivalent)
			- Moonlight (cool blue)
		- Presets set the color value — the user can further adjust after applying a preset

		**Design constraints:**
		- Color is specified as a hue, not as a color temperature in Kelvin — DPs think "warm" or "cool," not "3200K" in a previs context
		- Presets are named with the motivating source (candlelight, tungsten, daylight), not with technical values
	...
	- ##### 3.1.3. Light animation (Feature)
		...
		**Error cases:**
		``` python
			...
			# keyframe on a property that does not apply
			.if user keyframes a point light
			.if user later converts it to a directional light >>
				# this scenario should not arise — light type is fixed at creation
				# but if it did:
				!== position keyframes cause errors on a directional light
...
- ### Default scene lighting
	...
	**When the scene has zero user-placed lights:**
	...
	- The moment the user adds their first light, the default ambient reduces to a low level so user-placed lights dominate the scene
	- If the user deletes all lights, the default ambient returns to its full level
	...
	``` python
		...
		.if user adds a spot light >>
			<== default ambient dims to a low level
		...
		.if user deletes that light >>
			<== default ambient returns to full level
