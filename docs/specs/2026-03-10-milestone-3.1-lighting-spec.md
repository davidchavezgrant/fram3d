	...
	*Blocked by: 1.5 (keyframe animation — light animation uses the timeline system)*
	...
	- ##### 3.1.2. Light properties (Feature)
		...
		**Functional requirements:**
		...
		- Viewport icons/gizmos for lights (showing light position and type) are hideable — the user can toggle visibility of light HUD elements independently of object HUD elements
		...
		**Expected behavior:**
		``` python
			...
			# hiding light viewport icons
			.if user toggles light HUD visibility off >>
				<== all light viewport icons/gizmos are hidden
				<== object gizmos remain visible (independent toggle)
				<== lights still exist and illuminate the scene — only the icons are hidden
				||> .if user toggles light HUD visibility back on >>
					<== all light viewport icons reappear
		```
