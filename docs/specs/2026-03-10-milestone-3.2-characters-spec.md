  ...
  *Blocked by: 1.5 (keyframe animation -- characters animate on the timeline), 1.3 (scene management -- characters are scene elements that respond to selection, gizmos, and duplication)*
  ...
  - ##### 3.2.3. Custom Posing (Feature)
    ...
    **Prop Holding**
    ...
    Characters can hold objects. Prop attachment uses the **linking system** (3.3.1): the user links a prop to a character's hand body region. The prop follows the hand during posing and animation. Finger alignment is not precise — the object attaches to the hand area without detailed finger posing.

    The prop's **anchor point** (3.3.1) determines where on the prop the hand grips. For a sword, the anchor should be set to the handle. For a coffee cup, the grip area. The anchor point is set in the prop's inspector panel as an XYZ offset.

    ``` python
    .if >> user links a prop to a character's hand (via viewport drag or panel pickwhip — see 3.3.1)
    ...
    .if >> user unlinks a prop from a character's hand (via right-click → Unlink — see 3.3.1)
    ...
    .if >> user keyframes a character while a prop is linked to its hand
      <== the link state is separate from the character keyframe (links live on the global timeline — see 3.3.1)
  ...
  - ##### 3.2.4. Character Animation (Feature)
    ...
    *Related:
    ...
    - 1.5.3 (per-track stopwatch -- posing a character creates/updates keyframes when the stopwatch is on)*
    ...
    **Character Keyframes**
    ...
    .if >> user poses a mannequin while the shot is at time t
      ...
      <== stopwatch rules from 1.5.3 apply: near existing keyframe (0.1s) updates it, otherwise creates new
    ...
    **Animate Mode (Stopwatch) for Characters**
    ...
    .if >> user adjusts a joint (via joint indicators) while the shot is not playing
      <== stopwatch rules (1.5.3) apply to the full character state
