...
- ### 3.3. Object linking & grouping (Milestone)
	*Temporal object-to-character and object-to-object linking. Group objects for simultaneous transforms. The director puts a prop in a character's hand, passengers in a car, and plates on a table — and they move together.*
	...
	- **Grouping** — persistent multiselect. Objects in a group select and transform together, but there is no temporal behavior and no "following." Grouping is a convenience for simultaneous manipulation.
	...
	*Blocked by: 1.3 (scene management — linking extends the scene object model), 1.5 (keyframe animation — link/unlink events live on the global timeline)*
	...
	- ##### 3.3.2. Object Grouping (Feature)
		***Select multiple objects and group them. Selecting any member selects all. Transforming any member transforms all. Ungroup to break. No temporal behavior — grouping is a convenience for simultaneous manipulation.***
		...
		**Functional requirements:**
		...
		- Selecting any member of a group selects all members
		- Transforming any member (via gizmo or keyboard) transforms all members simultaneously, maintaining their relative positions
		- Ungroup: select a grouped object, right-click → "Ungroup" (or keyboard shortcut). All members become independent
		...
		``` python
			...
			.if "Plate", "Glass", "Fork" are in a group >>
				||> .if user clicks "Plate" in the viewport >>
					<== "Plate", "Glass", and "Fork" are all selected
					<== all three show selection highlights

			# Transforming a grouped object
			.if "Plate", "Glass", "Fork" are in a group >>
				||> .if user translates "Plate" by (2, 0, 0) >>
			...
			.if "Cup" is also in a group with "Saucer" >>
				...
				<== transforming "Cup" via gizmo is blocked while linked (linking rules override)
		```
