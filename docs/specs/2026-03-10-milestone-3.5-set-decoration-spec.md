# Milestone 3.5: Set Decoration Library
...
- ### 3.5. Set decoration library (Milestone)
	...
	*Blocked by: 2.3 (asset import — marketplace integration builds on the import pipeline established by model import and the basic asset library panel)*
	...
	- ##### 3.5.2. Marketplace integration (Feature)
		...
		***Browse and import 3D assets from existing online marketplaces directly within Fram3d — search, preview, download, and place without leaving the application.***

		This is the most ambitious feature in this milestone. Each marketplace (Sketchfab, TurboSquid, Unity Asset Store) has different APIs, authentication flows, licensing terms, file formats, and quality guarantees. A minimum viable version focuses on a single marketplace with the most accessible API and the fewest friction points. Subsequent phases add additional marketplaces.

		*Blocked by:
		- 2.3.1 (model import — marketplace assets require format conversion and collider generation)
		- 3.5.1 (built-in asset library — marketplace assets integrate into the same browsing UI)*

		*Related:
		- 3.5.3 (user asset management — downloaded marketplace assets can be organized into collections)*

		**Functional requirements:**

		*Marketplace browsing:*
		- The user can search marketplace catalogs from within the asset library panel
		- Marketplace search results display: thumbnail, name, author, price (or "Free"), and file size
		- The user can preview a marketplace asset before downloading — at minimum a larger thumbnail and basic metadata (polygon count, file format, license type)
		...
		*Download and import:*
		- The user can download a free or previously purchased asset with one click
		- Paid assets that the user has not purchased show a "Purchase on [marketplace]" action that opens the marketplace website — Fram3d does not handle payment
		...
		*Minimum viable scope:*
		- Phase 1: A single marketplace integration (whichever offers the most accessible API for free asset browsing and download)
		- Phase 2: Add remaining marketplaces one at a time, each behind its own authentication
		...
		**Design constraints:**
		...
		- Licensing information from the marketplace must be preserved with the downloaded asset so the user can check usage rights later
		...
		``` python
			...
			.if the user is authenticated with a marketplace >>
				...
				||> .if the user searches for "wooden table" >>
					<== marketplace results appear with thumbnails, names, authors, and prices
			...
			# Paid asset the user has not purchased
			.if the user finds a paid marketplace asset they have not purchased
			.if the user clicks the purchase action >>
				<== the marketplace website opens in the user's browser to the asset's purchase page
	...
	- ##### 3.5.3. User asset management (Feature)
		...
		*Related:
		- 3.5.1 (built-in asset library — built-in assets can be favorited and added to collections)
		- 3.5.2 (marketplace integration — downloaded marketplace assets appear in the user's asset inventory)
		...
		*Asset metadata:*
		...
		- Marketplace assets additionally display: author, license type
