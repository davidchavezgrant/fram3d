...
- ### 3.6. Multi-camera (Milestone)
  ...
  - ##### 3.6.1. Per-shot camera addition (Feature)
    ...
    **Functional requirements:**
    ...
    - Each camera is labeled with its letter (A, B, C, D) and assigned a color
    ...
    - Each camera preview element shows a small live thumbnail of that camera's current view
    ...
    - A newly added camera inherits the current viewport position, rotation, and focal length as its initial state
    ...
    - Removing a camera that has coverage segments assigned (3.6.4) causes those segments to be reassigned to Camera A
    ...
    **Error cases:**
    ``` python
      ...
      ||> .if user removes Camera B from the shot (3.6.1) >>
          <== Camera B's segment (2.0-3.5) is reassigned to Camera A
          <== coverage track now shows: A (0-2.0), A (2.0-3.5), A (3.5-5.0)
...
24. User removes Camera B from a shot that has coverage segments assigned to Camera B
    - <== Camera B's coverage segments are reassigned to Camera A
