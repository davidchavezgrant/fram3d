...
- ### 3.6. Multi-camera (Milestone)
  ...
  - ##### 3.6.1. Per-shot camera addition (Feature)
    ...
    **Functional requirements:**
    ...
    - When a user creates a new camera, the application prompts for camera body and lens choice, with an option to copy settings from the active camera or any other camera in the shot. This prompt has sensible defaults so the user can dismiss it quickly.
    - A newly added camera inherits the current viewport position, rotation, and focal length as its initial state (unless overridden by the creation prompt)
