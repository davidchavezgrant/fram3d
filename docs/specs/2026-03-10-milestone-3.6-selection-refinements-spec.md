# Milestone 3.6: Selection and Manipulation Refinements
...
- ### 3.6. Selection and manipulation refinements (Milestone)
	...
	*Blocked by: 1.3 (scene management -- selection and gizmos must exist), 1.5 (keyframe animation -- curves apply to keyframes)*
	...
	- ##### 3.6.1. Multi-select (Feature)
		...
		**Functional requirements:**
		...
		- Marquee selection: clicking and dragging on empty space draws a rectangular selection region on screen
			- All objects whose screen-space bounds intersect the rectangle are added to the selection
		...
		``` python
			...
			.if user clicks and drags on empty space (no modifier keys) >>
				...
				||> .if user releases mouse >>
					<== all objects intersecting the rectangle become selected
					<== objects outside the rectangle are not selected
	...
	- ##### 3.6.2. Grid snapping (Feature)
		...
		**Functional requirements:**
		...
		- Rotation snapping: when enabled, rotation via gizmo drag snaps to the nearest angle increment
			- Default rotation increment: 15 degrees
		...
		- Grid size is configurable by the user
			- Default: 0.5 meters
			- Available sizes: 0.1, 0.25, 0.5, 1.0, 2.0 meters
		...
		- Snapping applies equally to multi-select operations (3.6.1) -- the gizmo position snaps, and all selected objects move by the snapped offset
	...
	- ##### 3.6.3. Custom interpolation curves (Feature)
		***Control how the camera and objects ease between keyframes. The difference between a camera that mechanically slides from A to B and one that gently accelerates, glides, then settles into position -- the way a dolly grip actually pushes a dolly.***

		*Blocked by:
		- 1.5.4 (interpolation and playback -- the current system uses a single interpolation mode)*

		*Related:
		- 1.5.2 (tracks and keyframes -- curves are set per-keyframe)
		- 1.5.3 (keyframe interaction -- curve type is a property of a selected keyframe)*

		**Functional requirements:**
		- Every keyframe has an interpolation curve that controls how the animation transitions from that keyframe to the next
		...
		- Five curve presets:
			- **Linear**: constant speed from keyframe to keyframe. No acceleration, no deceleration. The current default behavior.
		...
		- Default interpolation for new keyframes: ease in-out
			- This replaces the current default (linear) because ease in-out produces more cinematic results out of the box
		- Selecting a keyframe in the timeline shows its current curve type
			- For presets (linear, ease in, ease out, ease in-out): a dropdown or button group to switch between them
			- For bezier: opens the curve editor
		- The curve type can be changed on multiple selected keyframes at once
			- Selecting several keyframes and choosing "Ease out" applies ease out to all of them
			- Bezier cannot be bulk-applied (each bezier curve is unique to its keyframe)

		**Curve editor:**
		- The curve editor displays a graph showing the interpolation curve for the selected keyframe
		...
		**Design constraints:**
		- The curve applies identically to all properties within a keyframe. A camera keyframe's position, rotation, and focal length all use the same easing curve for that segment. Per-property curves would add significant complexity for minimal creative benefit at this stage.
		...
		**Expected behavior:**
		``` python
			...
			.if user creates a new keyframe (any track) >>
				<== keyframe interpolation defaults to ease in-out
				!== keyframe interpolation defaults to linear

			# Preset selection
			.if user selects a keyframe in the timeline >>
				<== curve type indicator shows current type (e.g., "Ease In-Out")
				||> .if user switches to "Linear" >>
					<== playback between this keyframe and the next is constant speed
					<== no acceleration or deceleration

			# Ease in behavior
			.if keyframe A at t=0 has ease-in curve, keyframe B at t=2 >>
				||> .if playback runs from A to B >>
					<== movement starts slowly near keyframe A
					<== movement accelerates and reaches full speed arriving at keyframe B
					!== movement is constant speed throughout

			# Ease out behavior
			.if keyframe A at t=0 has ease-out curve, keyframe B at t=2 >>
				||> .if playback runs from A to B >>
					<== movement starts at full speed leaving keyframe A
					<== movement decelerates and arrives gently at keyframe B

			# Ease in-out behavior
			.if keyframe A at t=0 has ease-in-out curve, keyframe B at t=2 >>
				||> .if playback runs from A to B >>
					<== movement starts slowly near keyframe A
					<== movement reaches full speed in the middle
					<== movement decelerates approaching keyframe B

			# Bezier curve editing
			.if user selects a keyframe and switches to "Bezier" >>
				<== curve editor appears inline below the track
				...
				||> .if user drags a control handle >>
					...
					<== playback behavior updates to match the new curve

			# Curve editor for presets (read-only)
			.if user selects a keyframe with "Ease In" preset >>
				<== curve editor shows the ease-in curve shape
			...
			.if user selects keyframes A, B, and C >>
				||> .if user chooses "Ease Out" >>
					<== all three keyframes switch to ease out
					<== playback between each pair uses ease out

			# Per-segment independence
			.if track has keyframes A (ease in), B (linear), C (ease out) >>
				||> .if playback runs A to B to C >>
					<== A-to-B segment eases in
					<== B-to-C segment is linear
					!== all segments use the same curve

			# Last keyframe has no outgoing curve
			.if user selects the last keyframe in a track >>
				<== curve type controls are disabled or hidden
				<== no curve editor shown (no outgoing segment)
		```

		**Error cases:**
		``` python
			# Bezier handle constrained to valid range
			...
			.if a track has only one keyframe >>
				||> .if user selects it >>
					<== curve type controls disabled (no next keyframe to transition to)

			# Bezier bulk-apply blocked
			.if user selects multiple keyframes >>
				||> .if user attempts to switch to Bezier >>
...
1. User clicks object A, then Shift-clicks objects B and C.
   - <== three objects selected, gizmo at their center
   - <== all three show selection highlight

2. User drags a marquee around five objects in a furniture cluster.
   - <== all five selected in one operation
...
9. User enables rotation snapping at 15 degrees and rotates an object.
   - <== rotation jumps in 15-degree increments

### Custom interpolation curves

10. User creates three keyframes. Default interpolation is ease in-out.
    - <== playback between keyframes shows acceleration and deceleration

11. User selects a keyframe and switches to linear.
    - <== playback between that keyframe and the next is constant speed

12. User selects a keyframe and switches to bezier, then adjusts handles.
    - <== curve editor appears inline
    - <== dragging handles changes the curve shape
    - <== playback reflects the custom curve

13. User selects three keyframes and applies ease out.
    - <== all three keyframes switch to ease out in one operation

14. User selects the last keyframe in a track.
    - <== no curve type controls shown (no outgoing segment)
