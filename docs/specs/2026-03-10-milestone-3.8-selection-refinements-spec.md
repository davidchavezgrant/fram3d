# Milestone 3.8: Selection and Manipulation Refinements
...
- ### 3.8. Selection and manipulation refinements (Milestone)
	...
	*Blocked by: 1.3 (scene management -- selection and gizmos must exist), 1.5 (keyframe animation -- curves apply to keyframes)*
	...
	- ##### 3.5.1. Multi-select (Feature)
		...
		*Related:
		- 1.3.1 (scene elements -- extends single-select)
		- 1.3.2 (transform gizmos -- gizmos now operate on groups)
		- 3.4 (scene hierarchy -- parent/child groups are different from multi-select groups)*
		...
		**Functional requirements:**
		...
		- Clicking an object without Shift replaces the entire selection with that single object (existing 1.3.1 behavior, preserved)
		- Clicking empty space without Shift deselects everything (existing 1.3.1 behavior, preserved)
		...
		**Design constraints:**
		...
		- Selection highlighting applies independently to each selected object -- every selected object shows the selection highlight from 1.3.1
	...
	- ##### 3.5.2. Grid snapping (Feature)
		...
		*Related:
		- 1.3.2 (transform gizmos -- snapping applies during gizmo drags)
		- 1.3.3 (ground plane -- the visual grid overlay extends the ground plane grid)*
		...
		**Functional requirements:**
		...
		- Visual grid overlay:
			...
			- Grid lines are subtle and visually distinct from the ground plane's default reference grid (1.3.3)
		...
		- Snapping applies equally to multi-select operations (3.5.1) -- the gizmo position snaps, and all selected objects move by the snapped offset
	...
	- ##### 3.5.3. Custom interpolation curves (Feature)
		...
		*Blocked by:
		- 1.5.5 (interpolation and playback -- the current system uses a single interpolation mode)*

		*Related:
		- 1.5.2 (tracks and keyframes -- curves are set per-property on each keyframe)
		- 1.5.4 (keyframe interaction -- curve type is a property of a selected keyframe, configurable per animated property)*
