...
- ### 1.3. Scene management (Milestone)
	...
	**Delete behavior:** Context-sensitive delete uses two distinct shortcuts to avoid ambiguity:
	...
	- **Cmd+Delete (Ctrl+Delete on Windows)** — deletes the selected scene object entirely.
	This prevents the most common delete-ambiguity complaint from After Effects, Blender, and other tools where a single Delete key must guess whether you mean the keyframe or the object.
	...
	*Blocked by: 1.1 (virtual camera — need a viewport and camera to interact with objects)*
	...
	- ##### 1.3.5. Director view (Feature)
		***A global utility camera decoupled from the shot timeline. Lets the director orbit freely to see the full scene from any angle without affecting shot camera keyframes. Object manipulation still works — if the object's stopwatch is on, keyframes are created as normal.***

		*Related:
		- 1.1 (virtual camera rig — director view shows the shot camera as a frustum wireframe)
		- 1.5.3 (per-track stopwatch — object transforms in director view still create keyframes when the stopwatch is on)
		...
		**Functional requirements:**
		...
		- In director view, the user can still select and manipulate objects using gizmos
			- Object transform manipulations DO create keyframes if the object's stopwatch is on (Animate mode still applies to object transforms)
