	...
	*Blocked by: 1.5 (keyframe animation — light animation uses the timeline system)*
	...
	- ##### 3.1.3. Light animation (Feature)
		...
		*Related:
		...
		- 1.5.5 (interpolation and playback — light keyframes interpolate and evaluate during playback)
		- 1.5.3 (per-track stopwatch — light property changes create keyframes when the stopwatch is on)*
		...
		**Functional requirements:**
		...
		- Adding a keyframe to a light follows the same interaction model as camera and object keyframes:
			...
			- Animate mode (stopwatch on): changing a light property while the timeline is at a specific time creates or updates a keyframe at that time (same near-keyframe logic as objects, per 1.5.3)
		...
		- Light keyframes are selectable, draggable, and deletable using the same keyframe interaction model (1.5.4)
