# Milestone 3.4: Camera Follow and Look-At

**Date**: 2026-03-11
**Status**: Resolved
**Milestone**: 3.4 — Camera follow and look-at
**Project**: 3 — Production features
**Blocked by**: 3.2 (Characters — follow/look-at requires animated targets to be meaningful)

---

- ### 3.4. Camera follow and look-at (Milestone)
	*Camera maintains spatial relationship with a target. Timeline relationship model — not a mode to enter/exit. Implemented after characters (3.2), before snorricam.*
	...
	- ##### 3.4.1. Camera follow (Feature)
		...
		**Follow path:**
		- Camera's computed follow path visualized as a 3D spline (extends path visualization from 1.5.6)
		...
		**Interaction with existing systems:**
		...
		- Per-track stopwatch (1.5.3): Follow parameter changes (distance, height) create keyframes when stopwatch is on.
		...
		- Snorricam (3.5): Mutually exclusive. Snorricam is rigidly attached; follow maintains dynamic spatial relationship.
		- Multi-camera (3.9): A follow cam can be one camera in a multi-cam setup.
		- Shot sequencer (1.4): Follow is per-shot.
		- Export (2.4): Follow motion baked to keyframes during export (like camera shake).
	...
	**What this is NOT:**
	...
	- Not Snorricam (which is rigidly attached to the character's body)
